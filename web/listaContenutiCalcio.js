...
document.getElementById("listaCalcio").addEventListener("submit", submitForm);
...
promessa.then(nome => {
	...
	var vuoto = 0; //tutte le immagini hanno una spunta o su visibile o su nascosto
...
var vis = visibilita[i];
  ...
  var updateData = {
	mostra:vis,
				...
				//effettua modifiche
				var ref = firebase.database().ref("Json1").child(nomeCorrettoContenuto);
...
newdiv.innerHTML = '<br>'+'<center>'+"Selezionare le immagini da rendere visibili"+'</center>'+
                '<center>'+'<br>'+'<div class="alertNonSpuntato">'+"Una o più immagini non sono state spuntate"+'</div>'+'</center>'+
                '<br>'+'<div class="view_item" id="a'+i+'">'+
				'<div class="vi_left">'+
					'<img id="img'+i+'" src="">'+	
				'</div>'+
				'<div class="vi_right">'+
  ...
  '</div>'+
            ...
            newdiv.innerHTML = '<div class="view_item" id="a'+i+'">'+
				'<div class="vi_left">'+
					'<img id="img'+i+'" src="">'+	
				'</div>'+
				'<div class="vi_right">'+
...
promessa.then(nomiContenuti => {
	...
	location.href="modificaContenuto.html";
