...
document.getElementById("listaCommenti").addEventListener("submit", submitForm);
...
function submitForm(e) {
  ...
  const promessa = new Promise((resolve, reject) => {
  	var ref = firebase.database().ref("video").child("commenti");
	let nomeContenuto = [];
	let nome = [];
	let stato = [];
	...
	ref.on('value', function(snapshot) {
		snapshot.forEach(function(childSnapshot) {
			...
			nome.push(childSnapshot.val().nome);
...
var vis = visibilita[i];
  ...
  var updateData = {
	mostra:vis,
				...
				var ref = firebase.database().ref("video").child("commenti").child(nomeCorrettoContenuto);
				ref.on('value', function(snapshot) {
...
function mostraContenuti() {
	...
	ref.on('value', function(snapshot) {
		snapshot.forEach(function(childSnapshot) {
			...
			nomeIta.push(childSnapshot.val().nome);
...
function inserisciVideo(path, i) {
	...
	var storage = firebase.storage();
			var gsReference = storage.refFromURL("gs://play4health-abdc1.appspot.com/videos/commenti/"+path);
...
function ModificaVisibilaContenuto(elemento) {
	...
	ref.on('value', function(snapshot) {
		snapshot.forEach(function(childSnapshot) {
			...
			nomiContenuti.push(childSnapshot.val().nome);
...
promessa.then(nomiContenuti => {
	...
	location.href="modificaContenutoCommenti.html";
